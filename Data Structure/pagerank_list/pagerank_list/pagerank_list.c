...
double* calculatePageRank(double* vector, P_IALIST invAdjLists, int n, double d, int numIteration) {
	...
	double* prevPagerankVector = (double*)malloc(sizeof(double) * n);
	memcpy(prevPagerankVector, vector, sizeof(double) * n);

	for (int z = 0; z < numIteration; z++)
	{
		memset(vector, 0, sizeof(double) * n);

		// 2. sum ri/ci without no outlinks pages
		for (int i = 0; i < n; i++)
		{
			for (P_DATA pData = invAdjLists[i].first; pData != NULL; pData = pData->link)
			{
				int ci = invAdjLists[pData->pageID].numOutlinks;
				double ri = prevPagerankVector[pData->pageID];

				vector[i] += ri / (double)ci;
			}
